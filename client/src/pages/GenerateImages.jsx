              ...
              try {
                  ...
                  const prompt = `
                   ...
                   if(data.success ){
                     ...
                     toast.error(data.message || 'Could not generate image. Please try again later.')
                   }
               }
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {ImageStyle.map((item)=>  (
            <span onClick={()=> setSelectedStyle(item)} className={`text-xs px-4 py-1  border rounded-full cursor-pointer ${selectedStyle === item ? 'bg-green-100 text-green-700-700' : 'text-gray-500 border-gray-300 '}`} key={item}>{item}</span>
          ))}
        </div>
