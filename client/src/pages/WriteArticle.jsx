...
function WriteArticle() {
   ...
   const {getToken} = useAuth()
    ...
    const onSubmitHandler = async(e) =>{
        ...
        try{
          setLoading(true);
          const prompt = `Write an article about ${input} in ${selectedLength.text} words with proper paragraphs and headings.`
          ...
          const {data} = await axios.post('/api/ai/generate-article', {prompt , length:selectedLength.length}, {
           ...
           if (data.success && typeof data.content === "string") {
              ...
              console.error("Invalid response:", data);
               toast.error("Invalid article response");
             }
    ...
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        <div className='flex items-center gap-3'>
          <Sparkle className='w-6 text-red-900' />
        ...
        <br />
          <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#226BFF] to-[#65ADFF] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'> 
            {
              loading? <span className='w-4 h-4 my-1 rounded-full border-2 border-t-transparent anmate-spin'></span> 
              : <Edit className='w-5'/>
            }
