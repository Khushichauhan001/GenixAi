import { Edit, Eraser, Hash, Image, Scissors, Sparkle } from 'lucide-react'
      ...
      const {getToken} = useAuth() 
              ...
              const onSubmitHandler = async(e) =>{
                ...
                return toast.error("Please enter only single object name to remove");
                      ...
                      if (data.success) {
                       setContent(data.content); // content = image URL
                     } else {
                       ...
                       toast.error("Error removing object:", error.message);
                   } 
                       setLoading(false);
         ...
         <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
           ...
           />
             ...
             <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#417DF6] to-[#8E37EB] text-white px-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'> 
              {
                loading ? <span className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#41DF6] to-[#8E37EB] text-whitepx-4 py-2 mt-6 text-sm rounded-lg cursor-pointer'> </span> : <Scissors   className='w-5'/>
              }
