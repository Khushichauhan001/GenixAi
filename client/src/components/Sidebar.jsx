...
const Sidebar = ({ sidebar, setSidebar }) => {
  ...
  return (
    // container: fixed on small screens (overlay) and static on >=sm screens
    <aside
      className={
        `w-60 bg-white border-r border-gray-200 flex flex-col justify-between items-center
         ...
         ${sidebar ? 'translate-x-0' : ''}`  // when sidebar true (mobile) force translate-x-0
      }
      // on small screens we want it absolute overlay
      style={{ zIndex: 40 }}
    >
      <div className="my-7 w-full text-center">
        <img src={user?.imageUrl || '/fallback.png'} alt="User avatar" className="w-16 h-16 rounded-full mx-auto" />
        <h1 className="mt-2 text-center text-sm font-medium">{user?.fullName}</h1>
      </div>


      <div className='px-6 mb-7 text-sm text-gray-600 font-medium'>
        {
  ...
  {({ isActive }) => (
    <>
      ...
      <div className="mb-6 w-full">
        {/* example links */}
        <button className="block w-full text-left px-4 py-2 hover:bg-gray-100" onClick={() => { openUserProfile(); if (setSidebar) setSidebar(false) }}>
          Profile
        </button>
        <button className="block w-full text-left px-4 py-2 hover:bg-gray-100" onClick={() => signOut()}>
          Sign out
        </button>
      </div>
    </aside>
